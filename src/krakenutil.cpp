...
namespace kraken {
  ...
  bool check_tax_level(uint32_t call, string level, unordered_map<uint32_t, uint32_t> &parent_map,
  ...
  uint32_t resolve_tree2(unordered_map<uint32_t, uint32_t> &hit_counts,
    ...
    std::unordered_map<uint32_t, uint32_t>::const_iterator elem;
    ...
    while (it != hit_counts.end()) {
      ...
      uint32_t score = 0;
      while (node > 0) {
        elem = hit_counts.find(node);
        if (elem != hit_counts.end())
          score += elem->second;
        node = parent_map[node];
      }
...
/*KmerScanner::KmerScanner(string &seq, size_t start, size_t finish) {
  ...
  KmerScanner::KmerScanner(string &seq, size_t start, size_t finish, bool extend, uint32_t last_ambig,
                           uint64_t last_kmer) {
    if (!k)
      errx(EX_SOFTWARE, "KmerScanner created w/o setting k");
    if (finish > seq.size())
  ...
  uint32_t KmerScanner::get_ambig() {
    return ambig;
  }
